...
export function toHumanReadableFormat(input: string) {
  ...
  if (isAGM(cleanedInput)) {
    ...
    if (isDraft(cleanedInput)) {
      prefix = `<div class="inline-flex items-center rounded-md bg-blue-50 px-2 py-1 text-xs font-small text-blue-700 ring-1 ring-blue-700/10 ring-inset"> DRAFT </div>`;
    }

    return `${prefix} ${YYYY} AGM Meeting Minutes`;
  }
